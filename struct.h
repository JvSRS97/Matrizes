...
//Função para ler as entradas de uma matriz A
//----------------------------------------------------------------------------------------------------------------------------
matriz ler_matriz(FILE *arq, char nome_do_arquivo[]){
float V[400];
matriz A;
  ...
  if (arq == NULL){// Se houve erro na abertura
     ...
     exit(0);
		...
		int k=0;
		while((fscanf(arq,"%f ", &V[k])) != EOF){
				k+=1;
		}
		fclose(arq);

		int i,j;
		A.n = V[0]; //número de linhas
		A.m = V[1]; //número de colunas
		k=2;
		for(i=0;i<A.n;i++) 
			for(j=0;j<A.m;j++){
				A.entrada[i][j] = V[k];
...
//---------------------------------------------------------------------------
matriz Li_xLj(int i, int j, float x,matriz A){
	matriz C;
	int g;
	C = A;
	for(g=0;g<A.m;g++)
...
void decomp_LU(char nome_da_matriz[],matriz A){
		if(det(A) == 0)
			...
			matriz L,U;
			int i,j; 
			L = criar_id_n(A.m); //L inicialmente é a matriz identidade
			U = A; //U inicialmente é a matriz A
